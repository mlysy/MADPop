...
#                                                                                 #
#                                                                                 #
# Hierarchical Hardy-Weinberg analysis of variable alleled-fish                   #
#                                                                                 #
# code by mlysy and pjwkim, december 2014.                                        #
#                                                                                 #
# TUTORIAL                                                                        #
#                                                                                 #
# There are many steps in this tutorial that are labelled STEP 1 to STEP 6        #
# Each step has a "PURPOSE" written below for explanation of the step             #
# Throughout each of the steps there will be a "NOTE"'s as guide                  #
#                                                                                 #
#                                                                                 #
###################################################################################
...
#                                                                                 #
# STEP 0                                                                          #
# PURPOSE: Clean up dataset.  For a a preloaded dataset skip to step 1.           #
#                                                                                 #
###################################################################################
...
#                                                                                 #
# STEP 1                                                                          #
# PURPOSE: Calculate some summary statistics for the data, to (1) simulate        #
#          datasets which satisfy the Hardy-Weinberg equilibrium,                 #
#          and (2) draw MCMC samples for posterior inference.                     #
...
# In STEP 1, we have sourced all of the functions we will be using, and
# loaded the model, as well as the fish215.
# In STEP 2, we will be working with the lake data to fit models using STAN


###################################################################################
#                                                                                 #
# STEP 2                                                                          #
# PURPOSE: to make inferences for two lakes under the null hypothesis             #
#          Null hypothesis is that the two lakes are the same, and they           #
#          are not treated separately (grouped into one)                          #
#                                                                                 #
###################################################################################
...
# STEP 3 will take these rho observations from HW and plot the results
...
# Each plot shows the number of observations on y-axis,
#                         and the values on the x-axis.

# for each individual rho - for a set of inherited genotype (2 alleles)
# The red line shows the average (mean) value of the rho observations
# while the dotted red line represents the 95% C.I.
#nHobs <- Xdata$nH
nHobs <- nrow(Xsuff$H)
...
# Starting from STEP 4, we calculate the test statistics under chi-square/lrt.


###################################################################################
#                                                                                 #
# STEP 4                                                                          #
# PURPOSE: Now, calculate the chi-square and loglikelihood ratio test statistics  #
#           Please refer to notes on allele-functions to review:                  #
#               chi2.stat and LRT.stat functions                                  #
#                                                                                 #
###################################################################################
...
# Based on the threshold level, if the p-value is less than (or equal) to the
# significance level, then the null hypothesis is rejected.

# In this case, it will mean that the two lakes are not the same.
#
# In this section, we calculate the test statistics.
# A test statistic is a scalar function of all observations
# It is used to quantify factors within the observed data that would distinguish
# null hypothesis from the alternate hypothesis.
#
# NOTE: Testing deviations from Hardy Weinberg is generally performed using the Chi^2 test
#
#

# create a 2-way table with lakes as rows and observed genotypes as columns
# (each observed genotype is given a unique identifier)
...
# both fail to reject, but are very different in size.
# yet, they are supposed to give almost identical p-values when there is enough data
# for the chi^2 approximation to kick in...

# Please review the notes to the allele-functions file for explanation
# for how the test statistics are calculated.

#
#
#
# This is the end of STEP 4
# STEP 5 and 6 will be bootstrapping under unconstrained model
# and Hardy-Weinberg model respectively


###################################################################################
#                                                                                 #
# STEP 5a                                                                         #
# PURPOSE: bootstrapping under totally unconstrained model using MLE              #
#           2 methods will be used to bootstrap distribution of statistics        #
#               1) Unconstrained Model                                            #
#               2) Hardy-Weinberg Model (Step 6)                                  #
#                                                                                 #
###################################################################################
#
# Two models are used because there are limited samples.
# Limited samples may result in data with many empty genotype observations
# There are simply not even data to properly represent all the genotypes.
#
# Thus, we want to observe the results using an unconstrained model, and the
# Hardy-Weinberg constrained model, which is more conservative.
#
# The HW Principle states that allele and genotype frequencies in a population
# will remain constant from generation to generation w/o other influences
#
#

# First Method: totally Unconstrained Model
#   1.  Estimate the common probability of observing each genotype assuming both
#       lakes are the same
...
#   5.  Use the simulated histograms of chi2 and LRT to calculate the bootstrap p-values
#       for each test.
# NOTE: Bootstrapping refers to test that relies on random sampling with replacement
#             allows estimation of the sampling distribution of any statistic
...
# STEP 6 will now perform the same procedures, but under the Hardy-Weinberg model
#   (in constrast to the unconstrained model in STEP 5)


###################################################################################
#                                                                                 #
# STEP 6a                                                                         #
# PURPOSE: Same procedures as in STEP 5, but assuming the Hardy-Weinberg model    #
#           -Bootstrap by generating data from the HW model                       #
#                                                                                 #
###################################################################################

# Second Method: Hardy-Weinberg model
# NOTE: states that allele and genotype frequencies in a population will remain constant
# from generation to generation in the absence of other evolutionary influences.
# Much more conservative - since more constrained

# assuming 2 allele scenario:
# aa = p^2, Aa = 2p(1-p), AA = (1-p)^2
...
# display the posterior probabilities for each observed genotype
boxplot(rho.post)

# test for equality of populations almost exactly as for the multinomial bootstrap
# of section 5.
# the only difference is that instead of using a single value of p0 (the MLE under H0),
# we use a posterior distribution of p0's as fitted by the hierarchical model.
# so each of the bootstrap tables gets created with a different p0.
tab.obs <- UM.suff(Xobs)$tab
N <- rowSums(tab.obs)

nreps <- 1e4
system.time({
  boot.out3 <- UM.eqtest(N1 = N[1], N2 = N[2], p0 = rho.post, nreps = nreps)
})

# to display these nicely please reuse the code of the sections above.
par(mfrow = c(1,2))
stat.nm <- c("chi^2", "Lambda")
stat.obs <- c(chi2.stat(tab.obs), LRT.stat(tab.obs))
for(ii in 1:2) {
  hist(boot.out3[,ii], breaks = 100, freq = FALSE,
       main = parse(text = paste0("\"Distribution of \"*",
                      stat.nm[ii], "*\" Statistic under \"*H[0]")),
       xlab = parse(text = stat.nm[ii]))
  abline(v = stat.obs[ii], col = "red")
}
