...
$$
\pvb = \frac 1 M \sum*{m=1}^M \delta(T^{(m)} \ge T_\textrm{obs}).
$$
...
$$
In this sense, the posterior estimate of the probability vector $\rr_k$ can be non-zero even for genotypes for which $Y_{ki} = 0$, as long as $\alpha_i > 0$.  In practice, we estimate $\aa$ from the data $\Y$ from all $K$ populations (details in the following section).  The extent to which the genotypes observed in one lake affect inference in the other lakes is determined by $\alpha_0$ (the larger this value, the larger the effect).
